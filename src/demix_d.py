...
def get_matrix(spectrum_lib:dict, test_spectrum:pd.DataFrame, attr='normalized')->tuple:
    """Convert the given spectral library and unknown spectra to matrix output

    Args:
        spectrum_lib (dict): list of pandas dataframes
        test_spectrum (pd.DataFrame): dataframe of the unknown spectrum
        attr (str, optional): selected attribute.

    Returns:
        2d matrix: Returns two values, the first is the endmember matrix, the second is the test spectrum data
    """
...
def unmix_lasso(spectrum_lib:dict,unknown_spectrum:pd.DataFrame,attr='normalized',alpha=0.0005,max_mines=2,min_account=0.1)->tuple:
    """
